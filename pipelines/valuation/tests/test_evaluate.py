...
def test_both_real_tracks_score_item_five_from_the_reference_files_on_disk():
    # A count pinned to the live reference files breaks on every legitimate pool change; what the item
    # needs is that both halves actually score against real data, so assert the floor and the shape.
    for track, min_shares in (("optical", 1), ("ssb", 2)):
        ...
        assert out["share_difference"]["n"] >= min_shares, track
