...
ALLOW = [
    ...
    r"\|z\| = 2", r"\b0\.5\b", r"\b1%\b", r"\b99%\b", r"\b5 bps\b", r"\b20 bps\b", r"\b2%\b", r"\b98%\b",
    ...
    r"\bv[0-9]\b", r"\b19[0-9]{2}\b", r"\d{4}-\d{2}(-\d{2})?", r"\b(84|85|100|523|15)\b", r"\b30B\b", r"\b(27|31)B\b",
    r"\b(rank )?8\b", r"\b0\.001\b", r"\b12 (checks|of)", r"\b(p|t) (<|=|from|to)", r"\b\d{1,2}:\d{2}\b",
...
DATA_NUMBER = re.compile(r"(?<![\w.-])(?:\$\s?\d[\d,]*(?:\.\d+)?[MBK]?|\d[\d,]*(?:\.\d+)?\s?%|\d{1,3}(?:,\d{3})+|\d+\.\d+|\d{3,})(?![\w-])")


def prose(text: str) -> str:
    text = re.sub(r"^---\n.*?\n---\n", "", text, flags=re.S)  # frontmatter
    text = "\n".join(l for l in text.splitlines() if not re.match(r"^\s*(import|export)\b", l))
